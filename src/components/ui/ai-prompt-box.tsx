...
interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> { variant?: "default" | "outline" | "ghost"; size?: "default" | "sm" | "lg" | "icon"; }
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({ className, variant = "default", size = "default", ...props }, ref) => {
  const v = { default: "bg-white hover:bg-white/80 text-black", outline: "border border-[#444444] bg-transparent hover:bg-[#3A3A40]", ghost: "bg-transparent hover:bg-[#3A3A40]" };
    ...
    <div className={cn("flex flex-col items-center justify-center w-full transition-all duration-300 py-3", isRecording ? "opacity-100" : "opacity-0 h-0")}>
      ...
      <div className="w-full h-10 flex items-center justify-center gap-0.5 px-4">
        {[...Array(visualizerBars)].map((_, i) => (
          <div key={i} className="w-0.5 rounded-full bg-white/50 animate-pulse" style={{ height: `${Math.max(15, Math.random() * 100)}%`, animationDelay: `${i * 0.05}s`, animationDuration: `${0.5 + Math.random() * 0.5}s` }} />
        ))}
      </div>
    </div>
  );
...
interface PromptInputActionProps extends Omit<React.ComponentProps<typeof Tooltip>, "className"> { tooltip: React.ReactNode; children: React.ReactNode; side?: "top" | "bottom" | "left" | "right"; className?: string; }
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  ...
  const processFile = (file: File) => {
    ...
    r.onload = e => setFilePreviews({ [file.name]: e.target?.result as string });
  ...
  React.useEffect(() => {
    const handler = (e: ClipboardEvent) => {
      ...
      for (let i = 0; i < items.length; i++) {
        if (items[i].type.indexOf("image") !== -1) { const f = items[i].getAsFile(); if (f) { e.preventDefault(); processFile(f); break; } }
      }
    };
  ...
  return (
    <>
      <PromptInput value={input} onValueChange={setInput} isLoading={isLoading} onSubmit={handleSubmit}
        ...
        disabled={isLoading || isRecording} ref={ref as React.Ref<HTMLDivElement>}
      ...
      >
        ...
        {isRecording && <VoiceRecorder isRecording={isRecording} onStartRecording={() => {}} onStopRecording={d => { setIsRecording(false); onSend(`[Voice - ${d}s]`); }} />}
        ...
        <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
          <div className={cn("flex items-center gap-1 transition-opacity duration-300", isRecording ? "opacity-0 invisible h-0" : "opacity-100 visible")}>
            <PromptInputAction tooltip="Upload image">
              <button onClick={() => uploadRef.current?.click()} className="flex h-8 w-8 text-[#9CA3AF] items-center justify-center rounded-full hover:bg-gray-600/30 hover:text-[#D1D5DB] transition-colors">
                <Paperclip className="h-5 w-5" />
                <input ref={uploadRef} type="file" className="hidden" accept="image/*" onChange={e => { if (e.target.files?.[0]) processFile(e.target.files[0]); if (e.target) e.target.value = ""; }} />
              </button>
            </PromptInputAction>

            <div className="flex items-center">
              {[
                { key: "search", Icon: Globe, color: "#1EAEDB", show: showSearch, label: "Search" },
                { key: "think",  Icon: BrainCog, color: "#8B5CF6", show: showThink, label: "Think" },
                { key: "canvas", Icon: FolderCode, color: "#F97316", show: showCanvas, label: "Canvas" },
              ].map(({ key, Icon, color, show, label }, idx) => (
                <React.Fragment key={key}>
                  {idx > 0 && <CustomDivider />}
                  <button type="button"
                    onClick={() => { if (key === "search") { setShowSearch(p => !p); setShowThink(false); } else if (key === "think") { setShowThink(p => !p); setShowSearch(false); } else setShowCanvas(p => !p); }}
                    className={cn("rounded-full transition-all flex items-center gap-1 px-2 py-1 border h-8", show ? `border-current` : "bg-transparent border-transparent text-[#9CA3AF] hover:text-[#D1D5DB]")}
                    style={show ? { background: `${color}26`, borderColor: color, color } : undefined}
                  >
                    <div className="w-5 h-5 flex items-center justify-center">
                      <motion.div animate={{ rotate: show ? 360 : 0, scale: show ? 1.1 : 1 }} whileHover={{ rotate: show ? 360 : 15, scale: 1.1 }} transition={{ type: "spring", stiffness: 260, damping: 25 }}>
          ...
          <PromptInputAction tooltip={isLoading ? "Stop" : isRecording ? "Stop recording" : hasContent ? "Send" : "Voice"}>
            ...
            >
              {isLoading ? <Square className="h-4 w-4 fill-[#1F2023] animate-pulse" /> : isRecording ? <StopCircle className="h-5 w-5 text-red-500" /> : hasContent ? <ArrowUp className="h-4 w-4 text-[#1F2023]" /> : <Mic className="h-5 w-5 text-[#9CA3AF]" />}
            </Button>
          </PromptInputAction>
        </PromptInputActions>
      </PromptInput>
